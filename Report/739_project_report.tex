...
\usepackage{hyperref} 
\hypersetup{pdfinfo={
Title={Connect: A Distributed Communication Tool}, Author={Irtiza Ahmed Akhter,
Zainab Ghadiyal}, }}
...
\label{scalability} Next we talk about scalability of
the design. Once we designed the dynamic fault detection and recovery, we saw
...
up on line, the gliding in process showed in \textbf{Figure
...
\textbf{MySQL} instance and compared with others in real time under extreme
...
\begin{figure}[H] 
\centering
\includegraphics[scale=0.75]{Images/min_latency.PNG} 
\caption{\textbf{Min latency(ms) is independent of number of instances or users. We notice an outlier
case for 4 instances with 100 simulated users}} 
\label{fig:min_latency}
\end{figure} 

\begin{figure}[H] 
\centering
\includegraphics[scale=0.75]{Images/max_latency.PNG} \caption{\textbf{Max
latency(ms) increases with an in number of instances and users } }
\label{fig:max_latency} 
\end{figure} 

\section{Discussion} Apart from the scripts that we wrote for implementing the distributed environment, 
...
scripts inside each \emph{EC2} instance periodically checks the {Github}
